...
def perform_warmup(experiment_type, batch_size, device):
    ...
    try:
        ...
        temp_loader = get_dataloader(
            config["dataset_name"], config["transforms"], batch_size, train=True
        )
...
def run_supervised_experiments(experiments_to_run=None):
    ...
    for experiment_type in experiments_to_run:
        ...
        # Hyperparameter tuning is disabled (using pre-defined parameters)
        if False:
        ...
        run_experiments(
            ...
            optimizer_names=OPTIMIZERS,
            loss_title=f"Validation Loss ({config['plot_title']})",
            acc_title=f"Validation Accuracy ({config['plot_title']})",
            plot_filename=f"{experiment_type.lower()}_validation_curves",
            csv_filename=f"{experiment_type.lower()}_validation_metrics.csv",
            experiment_title=f"{config['plot_title']}",
            cost_xlimit=config.get("cost_xlimit"),
            f1_title=f"Validation F1 ({config['plot_title']})",
